...
public class Fox extends Hisniki {
    ...
    public Fox(int ves, int maxColvoNaCletce, int maxCletocZaHod, int maxKgEatDlyaSitostiZaXod) {
        this.ves = ves;
        ...
        for (int i = 0; i < Animal.objects.length - 1; i++) {
            for (int j = 0; j < Animal.objects[i].length; j++) {
                ...
                for (int k = 0; k < OstrovDemo.animals.size(); k++) {
                    if (OstrovDemo.animals.get(k) instanceof Fox) {
    ...
    public void eat() {
        ...
        while (itr.hasNext()) {
            ...
            if (x instanceof Zayac) {
                int b = ra * 70 / 100;
                if (b > 50) {
                    if (maxKgEatDlyaSitostiZaXod < 2) {
                        maxKgEatDlyaSitostiZaXod = maxKgEatDlyaSitostiZaXod + ((Zayac) x).getVes();
                        itr.remove();
                        Zayac.count--;
                        if (maxKgEatDlyaSitostiZaXod > 2) {//чтоб не переваливало за мах кол во жизней
                            maxKgEatDlyaSitostiZaXod = 2;
                        }
    ...
    public void razmno() {
        ...
        if (rand > 70) {
            if (maxKgEatDlyaSitostiZaXod == 2) {
                OstrovDemo.animals.add(new Fox(8, 30, 2, 1));
                count++;
            }
        }
    }
