...
public class Kangaroo extends Travoyadnie {
    ...
    public Kangaroo(int ves, int maxColvoNaCletce, int maxCletocZaHod, int maxKgEatDlyaSitostiZaXod) {
        this.ves = ves;
        ...
        for (int i = 0; i < Animal.objects.length - 1; i++) {
            for (int j = 0; j < Animal.objects[i].length; j++) {
                ...
                for (int k = 0; k < OstrovDemo.animals.size(); k++) {
                    if (OstrovDemo.animals.get(k) instanceof Kangaroo) {
    ...
    public void razmno() {
        ...
        if (rand > 50) {
            if (maxKgEatDlyaSitostiZaXod == 50) {
                OstrovDemo.animals.add(new Kangaroo(400, 50, 2, 1));
                count++;
            }
        }
    }
