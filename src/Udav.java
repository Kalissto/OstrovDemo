...
public class Udav extends Hisniki {
    ...
    public Udav(int ves, int maxColvoNaCletce, int maxCletocZaHod, int maxKgEatDlyaSitostiZaXod) {
        this.ves = ves;
        ...
        for (int i = 0; i < Animal.objects.length - 1; i++) {
            for (int j = 0; j < Animal.objects[i].length; j++) {
                ...
                for (int k = 0; k < OstrovDemo.animals.size(); k++) {
                    if (OstrovDemo.animals.get(k) instanceof Udav) {
    ...
    public void eat() {
        ...
        while (itr.hasNext()) {
            ...
            if (x instanceof Zayac) {
                int b = ra * 20 / 100;
                if (b > 50) {
                    if (maxKgEatDlyaSitostiZaXod < 3) {
                        maxKgEatDlyaSitostiZaXod = maxKgEatDlyaSitostiZaXod + ((Zayac) x).getVes();
                        itr.remove();
                        Zayac.count--;
                        if (maxKgEatDlyaSitostiZaXod > 3) {//чтоб не переваливало за мах кол во жизней
                            maxKgEatDlyaSitostiZaXod = 3;
                        }
    ...
    public void razmno() {
        ...
        if (rand > 50) {
            if (maxKgEatDlyaSitostiZaXod == 3) {
                OstrovDemo.animals.add(new Udav(15, 30, 1, 1));
                count++;
            }
        }
    }
