...
public class Horse extends Travoyadnie {
    private int ves;
    ...
    public Horse(int ves, int maxColvoNaCletce, int maxCletocZaHod, int maxKgEatDlyaSitostiZaXod) {
        this.ves = ves;
        ...
        for (int i = 0; i < Animal.objects.length - 1; i++) {
            for (int j = 0; j < Animal.objects[i].length; j++) {
                ...
                for (int k = 0; k < OstrovDemo.animals.size(); k++) {
                    if (OstrovDemo.animals.get(k) instanceof Horse) {
    ...
    public void eat() {
        ...
        while (itr.hasNext()) {
            ...
            if (x instanceof Grass) {
                if (maxKgEatDlyaSitostiZaXod < 60) {
                    maxKgEatDlyaSitostiZaXod = maxKgEatDlyaSitostiZaXod + ((Grass) x).getVes();
                    itr.remove();
                    Grass.count--;
                    if (maxKgEatDlyaSitostiZaXod > 60) {//чтоб не переваливало за мах кол во жизней
                        maxKgEatDlyaSitostiZaXod = 60;
                    }
    ...
    public void razmno() {
        ...
        if (rand > 50) {
            if (maxKgEatDlyaSitostiZaXod == 60) {
                OstrovDemo.animals.add(new Horse(400, 20, 4, 1));
                count++;
            }
        }
    }
