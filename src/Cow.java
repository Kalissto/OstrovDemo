...
public class Cow extends Travoyadnie {
    ...
    public Cow(int ves, int maxColvoNaCletce, int maxCletocZaHod, int maxKgEatDlyaSitostiZaXod) {
        this.ves = ves;
        ...
        for (int i = 0; i < Animal.objects.length - 1; i++) {
            for (int j = 0; j < Animal.objects[i].length; j++) {
                ...
                for (int k = 0; k < OstrovDemo.animals.size(); k++) {
                    if (OstrovDemo.animals.get(k) instanceof Cow) {
    ...
    public void eat() {
        ...
        while (itr.hasNext()) {
            ...
            if (x instanceof Grass) {
                if (maxKgEatDlyaSitostiZaXod < 100) {
                    maxKgEatDlyaSitostiZaXod = maxKgEatDlyaSitostiZaXod + ((Grass) x).getVes();
                    itr.remove();
                    Grass.count--;
                    if (maxKgEatDlyaSitostiZaXod > 100) {//чтоб не переваливало за мах кол во жизней
                        maxKgEatDlyaSitostiZaXod = 100;
                    }
    ...
    public void razmno() {
        ...
        if (rand > 50) {
            if (maxKgEatDlyaSitostiZaXod == 100) {
                OstrovDemo.animals.add(new Cow(700, 10, 3, 1));
                count++;
            }
        }
    }
