...
public class Duck extends Travoyadnie {
    ...
    public Duck(int ves, int maxColvoNaCletce, int maxCletocZaHod, int maxKgEatDlyaSitostiZaXod) {
        this.ves = ves;
        ...
        for (int i = 0; i < Animal.objects.length - 1; i++) {
            for (int j = 0; j < Animal.objects[i].length; j++) {
                ...
                for (int k = 0; k < OstrovDemo.animals.size(); k++) {
                    if (OstrovDemo.animals.get(k) instanceof Duck) {
    ...
    public void razmno() {
        ...
        if (rand > 80) {
            if (maxKgEatDlyaSitostiZaXod == 1) {
                OstrovDemo.animals.add(new Duck(1, 200, 4, 1));
                count++;
            }
        }
    }
